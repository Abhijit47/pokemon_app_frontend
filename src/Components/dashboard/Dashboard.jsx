...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const showUser = async () => {
      setIsLoading(true);
      try {
        const resUser = await axios.get(process.env.REACT_APP_GET_USER, options);
  ...
  const event = new Date(created_at).toLocaleDateString("en-IN", { timeZone: "Asia/Kolkata" });
        ...
        isLoading
          ...
          <>
            <div className="card mb-3 shadow-sm" style={{ maxWidth: "540px" }}>
              <div className="row gx-0">
                <div className="col-md-4">
                  <img src="https://placehold.co/600x600" className="img-fluid rounded-lg-start" alt="..." />
                </div>
            ...
            <div className='hstack flex-wrap justify-content-center -2 gap-2'>
              <Link to={"/create-pokemon"} className='btn btn-outline-primary shadow-sm'>
                Create your own
              </Link>
              <Link to={"/all-pokemons"} className='btn btn-outline-success shadow-sm'>
                Show your own
              </Link>
            </div>
          </>
