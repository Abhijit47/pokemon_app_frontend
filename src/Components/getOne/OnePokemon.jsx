...
const OnePokemon = () => {
  ...
  useEffect(() => {
    ...
    const showPokemon = async () => {
      setIsLoading(true);
      try {
        const resPokemon = await axios.get(`${process.env.REACT_APP_GET_ONE_POKEMON}/${id}`, options);
  ...
  const { name, id: iD, height, weight, base_experience: xp, order, held_items } = pokeDetails;
    ...
    isLoading
      ? (
        ...
        <div className='container-fluid p-3 mt-4 vstack align-items-center gap-3' >
          <div className="card shadow-sm bg-body-secondary" style={{ width: "18rem" }}>
            <div className='hstack justify-content-center mt-2'>
              <img src={front_shiny === undefined ? img : front_shiny} className="bg-body-tertiary w-50 h-50 rounded-circle shadow-sm" alt="..." />
            </div>
                  ...
                  return item.version_details?.map((details, i) => {
                    ...
                    return (
                      <ul key={i} className="list-group list-group-flush">
                        <li
                          className="list-group-item bg-dark-subtle text-capitalize border border-dark-subtle">
                          {`${i + 1}. ${name}`}
                        </li>
                      </ul>
                    );
          ...
          <div>
            <Link to={"/all-pokemons"} className="btn btn-outline-dark">Go Back</Link>
          </div>
        </div>
      )
  );
